    ...
    >
      ...
      >
        <div className={clsx("flex  items-center justify-center gap-x-3")}>
          <Box
            className={clsx(
              "bg-gradient-to-r from-green-500 to-green-400 w-10 h-10 rounded flex items-center justify-center"
            )}
          >
            <FaUserPlus size={20} className={clsx("text-white")} />
          </Box>
          <Typography tag="span" className={clsx("text-[18px] font-bold")}>
            جاب تلنت
          </Typography>
        </div>
        ...
        <div>
          <Button onClick={()=>handleLogin()} className={clsx("bg-green-500 text-white hover:bg-green-700")} size="xl">ورود</Button>
        </div>
      </div>
    </div>
  );
